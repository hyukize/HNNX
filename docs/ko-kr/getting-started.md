...
VS Code에서는 Explorer의 `.onnx` 파일을 선택합니다. HNNX가 기본 custom
editor로 열립니다. 다른 편집기가 선택되어 있다면 **Open With…**를 사용합니다.
...
- 노드, tensor, badge 또는 endpoint를 클릭하여 오른쪽 sidebar에서 확인합니다.
- sidebar 경계를 드래그하여 긴 tensor 이름을 표시합니다.
- 좌하단 정보 버튼에서 graph와 encoding 통계를 확인합니다.
- **View > Theme**에서 Auto, Light 또는 Dark를 선택합니다.

주요 toolbar는 다음 순서를 유지합니다.
...
AIMET Encodings…**, **Reload Encodings**, **Detach Encodings**에서 attachment를
...
VS Code remote session에서는 Mac이 아니라 remote extension host에 환경을
생성합니다.

`E`를 누르거나 `EDIT · BETA`를 선택합니다. 연결선을 선택하면 정확한 source와
destination을 확인할 수 있습니다. 예제 모델의 복사본을 편집하고 `INFER
...
- [AIMET precision 이해](aimet-encodings.md)
