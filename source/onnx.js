...
onnx.Graph = class {
    ...
    constructor(context, graph) {
        ...
        for (let inputIndex = 0; inputIndex < graph.input.length; inputIndex++) {
            const input = graph.input[inputIndex];
            const value = context.value(input.name);
            value.metadata = input.metadata || [];
            if (!value.initializer) {
                const argument = new onnx.Argument(input.name, [value]);
                argument._graph_edit_root = graph._graph_edit_root === true;
                argument._graph_edit_input_index = inputIndex;
                this._inputs.push(argument);
            }
        }
        for (let outputIndex = 0; outputIndex < graph.output.length; outputIndex++) {
            const output = graph.output[outputIndex];
            const value = context.value(output.name);
            value.metadata = output.metadata || [];
            if (!value.initializer) {
                const argument = new onnx.Argument(output.name, [value]);
                argument._graph_edit_root = graph._graph_edit_root === true;
                argument._graph_edit_output_index = outputIndex;
...
onnx.Value = class {
    ...
    get type() {
        return this._inferredType || this._type;
    }

    applyShapeInference(dataType, dimensions) {
        if (typeof dataType === 'string' && Array.isArray(dimensions)) {
            this._inferredType = new onnx.TensorType(dataType, new onnx.TensorShape(dimensions));
        }
    }
...
onnx.Node = class {
    ...
    constructor(context, node) {
        ...
        if (this.type && Array.isArray(this.type.inputs)) {
            for (let i = node.input.length; i < this.type.inputs.length && this.type.inputs[i].option === 'optional'; i++) {
                node.input.push(context.value(''));
                if (Array.isArray(node._graph_edit_input_indices)) {
                    node._graph_edit_input_indices.push(i);
                }
            }
        }
        for (let i = 0; i < node.input.length;) {
            ...
            const list = node.input.slice(i, i + count)
                .map((value, index) => ({ value, index: i + index }))
                .filter((entry) =>
                    entry.value.name !== '' ||
                    entry.value.initializer);
            const values = list.map((entry) => context.value(entry.value.name));
            const argument = new onnx.Argument(input.name, values);
            argument.option = input.option || null;
            let indices = list.map((entry) => entry.index);
            if (indices.length === 0 &&
                (input.option === 'optional' || node._graph_edit_root === true) && i < node.input.length) {
                indices = [i];
            }
            argument.source = {
                node,
                indices: indices.map((index) =>
                    Array.isArray(node._graph_edit_input_indices) ? node._graph_edit_input_indices[index] : index)
            };
        ...
        for (let i = 0; i < node.output.length;) {
            ...
            const list = node.output.slice(i, i + count)
                .map((value, index) => ({ value, index: i + index }))
                .filter((entry) => entry.value.name !== '' || entry.value.initializer);
            const values = list.map((entry) => context.value(entry.value.name));
...
onnx.Context.Model = class extends onnx.Context {
    ...
    constructor(metadata, locations, imports, graph, functions) {
        ...
        if (graph) {
            graph._graph_edit_root = true;
            for (let nodeIndex = 0; nodeIndex < graph.node.length; nodeIndex++) {
                const node = graph.node[nodeIndex];
                node._graph_edit_index = nodeIndex;
                node._graph_edit_input_indices = node.input.map((value, inputIndex) => inputIndex);
            }
            if (this._functions.size > 0) { // #1208
                ...
                while (graphs.size > 0) {
                    ...
                    for (const [index, node] of queue) {
                        ...
                        if (Array.isArray(node._graph_edit_input_indices)) {
                            node._graph_edit_input_indices.splice(index, 1);
                        }
                    }
                }
            }
...
onnx.Context.Graph = class extends onnx.Context {
    ...
    constructor(context, graph) {
        ...
        for (let nodeIndex = 0; nodeIndex < graph.node.length; nodeIndex++) {
            const node = graph.node[nodeIndex];
            node._graph_edit_root = graph._graph_edit_root === true;
            node._graph_edit_index = Number.isInteger(node._graph_edit_index) ? node._graph_edit_index : nodeIndex;
            node._graph_edit_input_indices = Array.isArray(node._graph_edit_input_indices) ?
                node._graph_edit_input_indices : node.input.map((value, inputIndex) => inputIndex);
