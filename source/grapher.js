...
grapher.Graph = class {
    ...
    setEdge(edge) {
        ...
        if (!this._edges.has(key)) {
            this._edges.set(key, { v: edge.v, w: edge.w, name: edge.name, label: edge });
        }
    }
    ...
    edge(v, w, name) {
        return this._edges.get(`${v}:${w}:${name || ''}`);
    }
    ...
    build(document, origin) {
        ...
        for (const edge of this.edges.values()) {
            ...
            if (edge.label.hitTest) {
                ...
                edge.label.hitTest.addEventListener('click', (event) => {
                    if (edge.label.activate) {
                        edge.label.activate();
    ...
    async layout(worker) {
        ...
        for (const edge of edges) {
            const label = this.edge(edge.v, edge.w, edge.name).label;
...
grapher.Edge = class {
    ...
    build(document, edgePathGroupElement, edgePathHitTestGroupElement, edgeLabelGroupElement) {
        ...
        if (this.id) {
            this.hitTest.setAttribute('id', `hit-${this.id}`);
        }
        ...
        if (this.label) {
            ...
            this.labelElement.setAttribute('class', this.labelClass ? `edge-label ${this.labelClass}` : 'edge-label');
            if (this.id) {
    ...
    update() {
        ...
        const smoothPath = (route) => {
            ...
            if (points.length < 2) {
                return { data: '', source: null, target: null };
            }
            return {
                data: new grapher.Edge.Curve(points).path.data,
                source: points[0],
        ...
        const path = Array.isArray(this.graphEditRoute) && this.graphEditRoute.length > 1 ?
            smoothPath(this.graphEditRoute) : curvePath(this, this.from, this.to);
        this.sourcePoint = path.source;
        this.targetPoint = path.target;
        this.element.setAttribute('d', path.data);
        this.hitTest.setAttribute('d', path.data);
