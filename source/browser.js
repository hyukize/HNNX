...
browser.Host = class {
    ...
    async execute(name) {
        switch (name) {
            ...
            case 'save-onnx-as':
            case 'infer-onnx-shapes': {
                throw new Error(
                    'NVIDIA ONNX GraphSurgeon editing and shape inference require the macOS app or VS Code extension.'
                );
            }
            default: {
                break;
    ...
    async _open(file, files) {
        this._view.show('welcome spinner');
        if (!file.name.toLowerCase().endsWith('.encodings')) {
    ...
    async _openFiles(files) {
        ...
        if (models.length > 0) {
            ...
            if (attachments.length > 0) {
                await this._open(attachments[0], files);
            }
        } else if (attachments.length > 0 && this._view.accept(attachments[0].name, attachments[0].size)) {
            await this._open(attachments[0], files);
        }
    }
...
browser.BrowserFileContext = class {
    ...
    async fetch(file, encoding, basename) {
        ...
        if (!blob) {
            if (typeof this._host.fetchWorkspace === 'function') {
                return this._host.fetchWorkspace(file, encoding);
            }
