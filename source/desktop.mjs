...
import {
    ...
    findOnnxGraphSurgeonPython,
    inferOnnxGraphSurgeonShapes
} from './onnx-graphsurgeon.js';
...
desktop.Host = class {
    ...
    async start() {
        ...
        this.document.body.addEventListener('drop', async (e) => {
            e.preventDefault();
    ...
    async save(name, extension, defaultPath) {
        return new Promise((resolve, reject) => {
            ...
            electron.ipcRenderer.send('show-save-dialog', {
                title: name === 'ONNX model' ? 'Save edited ONNX copy' : 'Export Tensor',
                defaultPath,
                buttonLabel: name === 'ONNX model' ? 'Save As' : 'Export',
    ...
    async execute(name, value) {
        if (name === 'save-onnx-as') {
            ...
            const target = await this.save('ONNX model', 'onnx', this._modelPath);
            if (!target) {
                this._view.graphEditSaveResult(null);
                return null;
            }
            try {
                if (Array.isArray(value.edits)) {
                    ...
                    const script = this._graphSurgeonScript();
                    await applyOnnxGraphSurgeonEdits(script, this._modelPath, target, value.edits, python);
                } else if (path.resolve(target) !== path.resolve(this._modelPath)) {
                    await fs.promises.copyFile(this._modelPath, target);
                }
                this._view.graphEditSaveResult({ path: target });
            } catch (error) {
                this._view.graphEditSaveResult({ error: error.message || String(error) });
                throw error;
            }
            return null;
        }
    ...
    _graphSurgeonScript() {
        ...
        let script = path.join(directory, 'onnx-graphsurgeon.py');
        const unpacked = script.replace(
            `${path.sep}app.asar${path.sep}`,
            `${path.sep}app.asar.unpacked${path.sep}`
        );
        if (unpacked !== script && fs.existsSync(unpacked)) {
            script = unpacked;
        }
        return script;
    }
