...
dagre.layout = (nodes, edges, layout, state) => {
    ...
    for (const edge of edges) {
        const label = g.edge(edge.v, edge.w, edge.name).label;
...
dagre.Graph = class {
    ...
    edge(v, w, name) {
        const edge = this.edges.get(this._edgeKey(this.directed, v, w, name));
        if (edge || name !== undefined) {
            return edge;
        }
        const node = this.nodes.get(v);
        return node ? node.out.find((item) => item.w === w) : undefined;
    }
